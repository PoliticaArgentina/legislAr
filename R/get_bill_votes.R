...
No se detecto acceso a internet. Por favor chequear la conexion.")
  ...
  url2 <- glue::glue("https://raw.githubusercontent.com/electorArg/PolAr_Data/master/legis/{selection$chamber}/votos/{selection$year}/{selection$id}.json") 
  
  check2 <- httr::GET(url2)
  
  httr::stop_for_status(x = check2, 
                        task = "Fail to download election data. Source is not available // La fuente de datos electorales no esta disponible")
  
  
  
  
jsonlite::read_json(url2) %>% 
  tibble::enframe() %>% 
